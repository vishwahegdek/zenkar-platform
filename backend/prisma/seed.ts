...
import { PrismaClient, OrderStatus, DeliveryStatus, PaymentStatus, ItemStatus } from '@prisma/client';
...
async function main() {
  ...
  // Only seed admin if usage is strictly controlled or desired in production
  // For now, let's assume Admin is essential. 
  // Ideally, use a safer check or Environment variables for Admin Pass.
  ...
  // Seed Expense Categories (Essential)
  ...
  for (const cat of categories) {
    ...
    if (!exists) {
      ...
      console.log(`Created expense category: ${cat}`);
    }
  }

  // Seed Product Categories (Essential)
  let generalCategory = await (prisma as any).productCategory.findFirst({ where: { name: 'General' } });
  if (!generalCategory) {
      generalCategory = await (prisma as any).productCategory.create({ data: { name: 'General' } });
      console.log('Created default product category: General');
  }

  // Seed "Bee boxes" (Essential)
  let beeBoxes = await (prisma as any).productCategory.findFirst({ where: { name: 'Bee boxes' } });
  if (!beeBoxes) {
      beeBoxes = await (prisma as any).productCategory.create({ data: { name: 'Bee boxes' } });
      console.log('Created product category: Bee boxes');
  }
      ...
      for (const c of customers) {
          ...
          if (!exists) {
              await prisma.customer.create({
                  data: { ...c, userId: existingUser?.id || 1 } 
              });
