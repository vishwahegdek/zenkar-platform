...
describe('Finance Module (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    if (loginRes.status !== 201 && loginRes.status !== 200) {
        ...
        await request(app.getHttpServer()).post('/auth/register').send({ username: 'financetest', password: 'password123' });
        loginRes = await request(app.getHttpServer()).post('/auth/login').send({ username: 'financetest', password: 'password123' });
