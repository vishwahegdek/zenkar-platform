...
describe('LabourSystem Comprehensive Coverage (e2e)', () => {
  ...
  it('2. Enter Random Data (15 Days)', async () => {
    ...
    console.log(
      `    -> Generated 15 days data. Expected Days: ${expectedDays}, Expected Paid: ${expectedPaid}`,
    );
  });
  ...
  it('3. Cross Check Pre-Settlement Report', async () => {
    ...
    console.log(
      `    -> Checking Report: Days ${report.totalDays} vs ${expectedDays}`,
    );
  ...
  it('4. Settle on Day 10', async () => {
    ...
    expectedPaid = 0;
    daysData.forEach((d) => {
      if (d.date > SETTLEMENT_DATE) {
        // Strict Greater Than for Active Report
        expectedDays += d.attendance;
        expectedPaid += d.amount;
      }
    });
    console.log(
      `    -> New Period Expectations: Days ${expectedDays}, Paid ${expectedPaid}`,
    );
  });
  ...
  it('5b. Verify Immutable Settlements (Partial Update)', async () => {
    ...
    console.log(
      `    -> Attempting to modify settled data on ${settledDate} (Should be ignored)`,
    );
    ...
    expect(Number(report.balance)).toBe(balance);
    console.log(
      '    -> Verified: Totals remained unchanged after attempted modification of settled data',
    );
  });
  ...
  it('5c. Verify Carry Forward & History', async () => {
    ...
    console.log(
      `    -> Verified OB: ${report.openingBalance}, Balance: ${report.balance}`,
    );
