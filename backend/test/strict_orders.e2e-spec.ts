...
describe('Strict Order Module (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    app.useGlobalPipes(new ValidationPipe({ whitelist: true, transform: true })); // Mimic Main.ts
    await app.init();
    ...
    if (loginRes.status !== 201 && loginRes.status !== 200) {
        ...
        await request(app.getHttpServer()).post('/auth/register').send({ username: 'testuser', password: 'password123' });
        const res = await request(app.getHttpServer()).post('/auth/login').send({ username: 'testuser', password: 'password123' });
  ...
  describe('Step 1: Contacts Module', () => {
    it('should fail if PHONE is missing', async () => {
       await request(app.getHttpServer())
        ...
        .expect(400)
        .expect(res => {
            if (!JSON.stringify(res.body).includes('phone')) throw new Error('Error message missing "phone" validation');
        });
    });
  ...
  describe('Step 3: Products Module', () => {
      it('should fail if name missing', async () => {
          await request(app.getHttpServer())
            .post('/products')
            .set('Authorization', `Bearer ${authToken}`)
            .send({ defaultUnitPrice: 10 }).expect(400);
      });
      it('should fail if defaultUnitPrice missing', async () => {
         await request(app.getHttpServer())
            .post('/products')
            .set('Authorization', `Bearer ${authToken}`)
            .send({ name: 'Test Prod' }).expect(400);
      });
  ...
  afterAll(async () => {
      ...
      if (createdContactId) {
          await request(app.getHttpServer()).delete(`/contacts/${createdContactId}`).set('Authorization', `Bearer ${authToken}`).expect(200);
      }
      if (createdCustomerId) {
          await request(app.getHttpServer()).delete(`/customers/${createdCustomerId}`).set('Authorization', `Bearer ${authToken}`).expect(200);
      }
      if (createdProductId) {
          await request(app.getHttpServer()).delete(`/products/${createdProductId}`).set('Authorization', `Bearer ${authToken}`).expect(200);
      }
    await app.close();
  });
});
